...
require 'inc.con.php';
  ...
  if ( $_POST['action'] == "datatable") {
	    ...
	    $sql = " SELECT destinations.dest_id, destinations.destination FROM  destinations where 1=1";		
		
		$columns = array('destinations.dest_id', 'destinations.destination' );
...
{ // print_r($_FILES);
	...
	if($_POST['dest_id']=="")
	{
		if(!($obj->checkExiststance2("destinations","destination",$_POST['destination'])))
		{
	         $status = false;
		...
		{
		  $id=$obj->get_max("destinations","dest_id");
			
			 
	       		$query="insert into destinations values(:id,:destination)";
		$param= array(
			'id' => $id,
	...
	else
	{
					...
					$query="update destinations set destination=:destination where dest_id=:id";
					$param= array(
					'id' => $_POST['dest_id'],
					'destination' =>$_POST['destination']				
...
if (isset($_POST['action']) && $_POST['action'] == "edit") {
		...
		$sql = "SELECT destinations.dest_id, destinations.destination FROM  destinations WHERE dest_id={$id}";
		$param = array($id);
	...
	if (isset($_POST['action']) && $_POST['action'] == "delete") {
		extract(array_map("filterIn", $_POST));
		$isDeleted = $obj->delete('destinations', 'dest_id', $id);
		if ($isDeleted) {
			$status = true;
