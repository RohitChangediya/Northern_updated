 ...
 fetchdestinations();
      ...
      clearFields();
		...
		$("#submit").on('click', function(event) {
	     ...
	     if( $("#transport").val()=="") {
	     	$("#transport").focus();
	     	alert("Please Enter valid transport.....!");
	     	return;
	     }
		 ...
		 if( $("#transport").val()=="")
	      ...
	      //console.log($form_data);
		    ...
		    var $request = $.ajax({
		                          url :'inc/maintain_transport.php',
		                          type: "POST",
		                          data: $form_data,								
								  dataType: 'json',
...
$("#trans_id").val($resp[0].trans_id);
 $("#transport").val($resp[0].transport);
 $("#arrive_dest_id").val($resp[0].arrive_dest_id);
 $("#dep_dest_id").val($resp[0].dep_dest_id);
   ...
   var $request = $.ajax({
            ...
            $request.done(function(data) {
              //if success close modal and reload ajax table
                 //console.log(data);
                  if (data.status) {
                     $("#arrive_dest_id").html(data.msg);
                    $("#dep_dest_id").html(data.msg);
